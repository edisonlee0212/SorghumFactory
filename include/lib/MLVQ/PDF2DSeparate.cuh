	...
	struct PDF2DSeparate {
		...
		struct PDF2DLuminance
		{
				...
				void GetVal(const int& pdf2DIndex, glm::vec3& out, SharedCoordinates& tc, const PDF1D& pdf1, const bool& print) const
			{
				...
				const float l1 = m_pdf2DScales[pdf2DIndex * m_numOfAlpha + i] * pdf1.GetVal(m_pdf2DSlices[pdf2DIndex * m_numOfAlpha + i], tc);
				const float l2 = m_pdf2DScales[pdf2DIndex * m_numOfAlpha + i + 1] * pdf1.GetVal(m_pdf2DSlices[pdf2DIndex * m_numOfAlpha + i + 1], tc);
				out[0] = (1.f - w) * l1 + w * l2;
			}
		};
