...
namespace RayTracerFacility
{
	struct SharedCoordinates
	{
		...
		SharedCoordinates() {}
		SharedCoordinates(
			const bool& useBtfFlag, const bool& useCosBeta, 
		...
		__device__ void SetForAngleBetaDeg(const float& beta)
		{
			...
			if (m_useCosBeta) {
				m_currentBetaLowBound = glm::clamp(
					static_cast<int>((glm::sin(glm::radians(m_beta)) + 1.0f) / 2.0f * (m_numOfBeta - 1)),
					0, m_numOfBeta - 2);
				m_weightBeta = (m_beta - m_betaAngles[m_currentBetaLowBound]) / (m_betaAngles[m_currentBetaLowBound + 1] - m_betaAngles[m_currentBetaLowBound]);
		...
		__device__ void SetForAngleAlphaDeg(const float& alpha)
		{
			...
			m_alpha = alpha;
			m_currentAlphaLowBound = glm::clamp(static_cast<int>((m_alpha + 90.0f) / m_stepAlpha), 0, m_numOfAlpha - 2);
			m_weightAlpha = (m_alpha + 90.f - m_currentAlphaLowBound * m_stepAlpha) / m_stepAlpha;
		...
		__device__ void SetForAnglePhiDeg(const float& phi)
		{
			...
			m_phi = phi;
			m_currentPhiLowBound = glm::clamp(static_cast<int>(m_phi / m_stepPhi), 0, m_numOfPhi - 1);
		...
		__device__ void SetForAngleThetaDeg(const float& theta)
		{
			...
			m_theta = theta;
			m_currentThetaLowBound = glm::clamp(static_cast<int>(m_theta / m_stepTheta), 0, m_numOfTheta - 2);
			m_weightTheta = (m_theta - m_currentThetaLowBound * m_stepTheta) / m_stepTheta;
			assert(m_weightTheta > -0.001f && m_weightTheta < 1.001f);
		}

	};
	__device__ inline void ConvertThetaPhiToBetaAlpha(const float theta, const float phi,
