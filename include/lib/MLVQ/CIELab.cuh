...
namespace RayTracerFacility {
	/*! YCbCr to RGB space */
	__device__
		inline void YCbCrToRgb(const glm::vec3& yCbCr, glm::vec3& rgb)
	{
		assert(yCbCr[0] >= 0.f);
		assert(yCbCr[1] >= 0.f);
		assert(yCbCr[2] >= 0.f);
	...
	/*! YCbCr to RGB space */
	__device__
		inline void YCbCrToRgbNormalized(const glm::vec3& yCbCr, glm::vec3& rgb)
	{
		assert(yCbCr[0] >= 0.f);
		assert(yCbCr[1] >= 0.f);
		assert(yCbCr[2] >= 0.f);
	...
	__device__
		inline int LogLuv2Rgb(const glm::vec3& luv, glm::vec3& rgb)
	{
	...
	__device__
		inline int LogLuv2RgbNormalized(const glm::vec3& luv, glm::vec3& rgb)
	{
		/*! \brief Conversion from LogLu'v' (not L*u*v*)  colour space according to [Ward98] into
		  RGB colourspace for Observer. = 2degree, Illuminant = D65 */
	...
	__device__
		inline void UserCmToRgb(const glm::vec3& userColorModelData,
			glm::vec3& rgb,
