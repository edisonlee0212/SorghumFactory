...
namespace RayTracerFacility {
	...
	public:
		...
		template<typename T>
		void Upload(T* t, const size_t& count);
		template<typename T>
		void Download(T* t, const size_t& count);
	};
	template <typename T>
	void CudaBuffer::Upload(std::vector<T>& data)
	{
		Upload(data.data(), data.size());
	}

	template <typename T>
	void CudaBuffer::Upload(T* t, const size_t& count)
	{
		Upload(t, sizeof(T), count);
	}

	template <typename T>
	void CudaBuffer::Download(T* t, const size_t& count)
	{
