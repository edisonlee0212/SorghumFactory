...
namespace PlantFactory {
	...
	{
	public:
		virtual void GenerateField(std::vector<std::vector<glm::mat4>>& matricesList) {};
	};
	...
	public:
		...
		void GenerateField(std::vector<std::vector<glm::mat4>>& matricesList) override;
	};
	...
	protected:
		...
		static void ObjExportHelper(glm::vec3 position, std::shared_ptr<Mesh> mesh, std::ofstream& of, unsigned& startIndex);
	public:
...
#pragma endregion
		...
		static void OnGui();
		static void Update();
		static void CreateGrid(SorghumField& field, const std::vector<Entity>& candidates);
		static void CloneSorghums(const Entity& parent, const Entity& original, std::vector<glm::mat4>& matrices);
		static void ExportSorghum(const Entity& sorghum, std::ofstream& of, unsigned& startIndex);
