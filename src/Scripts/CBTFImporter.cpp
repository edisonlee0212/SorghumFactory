...
#endif
void Scripts::CBTFImporter::OnInspect() {
  ...
  ImGui::Text(
      ("Remaining Folders: " + std::to_string(m_importFolders.size())).c_str());
  ...
  if(m_processing){
    ...
    if(!m_importFolders.empty() && ImGui::Button("Clear")) m_importFolders.clear();
  }
}
void Scripts::CBTFImporter::Update() {
