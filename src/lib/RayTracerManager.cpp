    ...
    if (const auto *rayTracedEntities = EntityManager::UnsafeGetPrivateComponentOwnersList<RayTracedRenderer>(); rayTracedEntities) {
        for (auto entity : *rayTracedEntities) {
            ...
            for (auto &currentRayTracerInstance : meshesStorage) {
                if (currentRayTracerInstance.m_entityId == entity.GetIndex() &&
                    ...
                    if (rayTracerInstance->m_surfaceColor != rayTracedRenderer.m_surfaceColor
                        || rayTracerInstance->m_metallic != (rayTracedRenderer.m_metallic == 1.0f ? -1.0f : 1.0f /
                                                                                                            glm::pow(
                                                                                                                    1.0f -
                                                                                                                    rayTracedRenderer.m_metallic,
                                                                                                                    3.0f))
            ...
            if (fromNew || needVerticesUpdate || needTransformUpdate || needMaterialUpdate) {
                ...
                rayTracerInstance->m_metallic = rayTracedRenderer.m_metallic == 1.0f ? -1.0f : 1.0f / glm::pow(
                        1.0f - rayTracedRenderer.m_metallic, 3.0f);
                rayTracerInstance->m_roughness = rayTracedRenderer.m_roughness;
                rayTracerInstance->m_enableMLVQ = rayTracedRenderer.m_enableMLVQ;
                rayTracerInstance->m_mlvqMaterialIndex = rayTracedRenderer.m_mlvqMaterialIndex;
            ...
            if (rayTracedRenderer.m_albedoTexture &&
                rayTracedRenderer.m_albedoTexture->Texture()->Id() != rayTracerInstance->m_albedoTexture) {
                updateShaderBindingTable = true;
            ...
            if (rayTracedRenderer.m_normalTexture &&
                rayTracedRenderer.m_normalTexture->Texture()->Id() != rayTracerInstance->m_normalTexture) {
                updateShaderBindingTable = true;
...
void RayTracerManager::Update() {
...

    {
        ...
        manager.m_defaultRenderingProperties.m_camera.Set(EditorManager::GetInstance().m_sceneCameraRotation,
        ...
        if (!CudaModule::GetRayTracer()->m_instances.empty()) {
            manager.m_defaultWindow.m_rendered = CudaModule::GetRayTracer()->RenderDefault(
...
void RayTracerRenderWindow::OnGui() {
    ...
    ImGui::Begin(m_name.c_str());
    {
        if (ImGui::BeginChild("CameraRenderer", ImVec2(0, 0), false,
            ...
            if (m_rendered)
                ImGui::Image(reinterpret_cast<ImTextureID>(m_output->Id()), viewPortSize, ImVec2(0, 1), ImVec2(1, 0));
            else ImGui::Text("No mesh in the scene!");
            if (ImGui::IsWindowFocused()) {
                const bool valid = true;
                const glm::vec2 mousePosition = InputManager::GetMouseAbsolutePositionInternal(
...
#pragma region Scene Camera Controller
                    ...
                    if (m_rightMouseButtonHold && !EditorManager::GetInstance().m_lockCamera) {
                        const glm::vec3 front =
                                EditorManager::GetInstance().m_sceneCameraRotation * glm::vec3(0, 0, -1);
