...
ProceduralPinnacleState ProceduralPinnacleDescriptor::Get(float time, unsigned seed) const {
...
ProceduralStemState ProceduralStemDescriptor::Get(float time, unsigned seed) const {
...
ProceduralLeafState ProceduralLeafDescriptor::Get(float time, unsigned seed) const {
...
ProceduralSorghumState ProceduralSorghumDescriptor::Get(float time) const {
...
void ProceduralSorghumDescriptor::OnInspect() {
  ...
  if (ImGui::Button("Instantiate")) {
    Application::GetLayer<SorghumLayer>()->CreateSorghum(
        AssetManager::Get<ProceduralSorghumDescriptor>(GetHandle()));
  }
...
void ProceduralSorghumDescriptor::Set(
...
void ProceduralSorghumDescriptor::Set(
  ...
  float growthTimeForLastLeaf =
      growthTimeForLongestLeaf * m_leaves.back().m_maxLength / maxLength;
  float startingTimeStep =
      leafSize <= 1 ? 0.0f : (1.0f - growthTimeForLastLeaf) / (leafSize - 1);
  for (int i = 0; i < leafSize; i++) {
    ...
    float growthTime =
        leaf.m_maxLength / maxLength * growthTimeForLongestLeaf * endTime;
    leaf.m_startTime =
        glm::clamp(startingTimeStep * (float)i * endTime, 0.0f, 1.0f);
    leaf.m_endTime = glm::clamp(leaf.m_startTime + growthTime, 0.0f, 1.0f);
  }
}
bool ProceduralLeafDescriptor::OnInspect(float maxTime) {
  bool changed = false;
  if (ImGui::SliderFloat("Start time", &m_startTime, 0.0f, maxTime))
    changed = true;
...
bool ProceduralPinnacleDescriptor::OnInspect(float maxTime) {
  ...
  if (ImGui::SliderFloat("Start time", &m_startTime, 0.0f, maxTime))
    changed = true;
  if (ImGui::DragFloat("End time", &m_endTime, 0.0f, maxTime))
    changed = true;
...
void ProceduralSorghumDescriptor::Serialize(YAML::Emitter &out) {
...
void ProceduralSorghumDescriptor::Deserialize(const YAML::Node &in) {
  if (in["m_endTime"])
