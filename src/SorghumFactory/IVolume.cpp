#include <IVolume.hpp>
glm::vec3 PlantArchitect::SphericalVolume::GetRandomPoint() {
  return glm::ballRand(1.0f) * m_radius;
}
bool PlantArchitect::SphericalVolume::InVolume(
    const GlobalTransform &globalTransform, const glm::vec3 &position) {
  return false;
}
bool PlantArchitect::SphericalVolume::InVolume(const glm::vec3 &position) {
