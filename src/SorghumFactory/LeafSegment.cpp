...
LeafSegment::LeafSegment(glm::vec3 position, glm::vec3 up, glm::vec3 front, float leafHalfWidth,
                         float theta, bool isLeaf, float surfacePush,
                         float leftFlatness, float rightFlatness,
...
glm::vec3 LeafSegment::GetPoint(float angle) {
    if (m_theta < 90.0f) {
        ...
        const auto center = m_position + (m_radius - maxHeight) * m_up;
        ...
        return center - m_radius * direction - actualHeight * compressFactor * m_up - maxHeight * m_up * m_surfacePush;
    }
    const auto direction = glm::rotate(m_up, glm::radians(angle), m_front);
