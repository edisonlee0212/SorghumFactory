...
void SorghumStateGenerator::Deserialize(const YAML::Node &in) {
  ...
  if (in["m_leafDescriptors"]) {
    for (const auto &i : in["m_leafDescriptors"]) {
      m_leafDescriptors.push_back(SorghumLeafGenerator());
...
void SorghumStateGenerator::L1ToBase() {
...
void SorghumStateGenerator::Ready() {
  if (m_cascadeIndex == 1) {
    L1ToBase();
  }
}
void SorghumStemGenerator::Serialize(YAML::Emitter &out) {
...
void SorghumStemGenerator::Deserialize(const YAML::Node &in) {
...
bool SorghumStemGenerator::OnInspect() {
  bool changed = false;
...
bool SorghumLeafGenerator::OnInspect() {
  bool changed = false;
  changed = changed ||
...
void SorghumLeafGenerator::Serialize(YAML::Emitter &out) {
...
void SorghumLeafGenerator::Deserialize(const YAML::Node &in) {
...
bool SorghumPinnacleGenerator::OnInspect() {
  bool changed = false;
...
void SorghumPinnacleGenerator::Serialize(YAML::Emitter &out) {
...
void SorghumPinnacleGenerator::Deserialize(const YAML::Node &in) {
