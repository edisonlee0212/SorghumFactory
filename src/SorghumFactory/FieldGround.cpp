...
void SorghumFactory::FieldGround::GenerateMesh(float overrideDepth) {
  ...
  for (int i = -m_size.x; i <= m_size.x; i++) {
    for (int j = -m_size.y; j <= m_size.y; j++) {
      ...
      archetype.m_position.y = glm::min(
          0.0f,
          (overrideDepth < 0.0f ? m_alleyDepth : overrideDepth) * glm::cos(archetype.m_position.z * m_rowWidth));

      float noise = glm::simplex(m_noiseScale * archetype.m_position) * m_noiseIntensity;
      archetype.m_position.y += noise;
      archetype.m_texCoords =
          glm::vec2((float)i / (2 * m_size.x + 1), (float)j / (2 * m_size.y + 1));
  ...
  for (int i = 0; i < 2 * m_size.x; i++) {
    for (int j = 0; j < 2 * m_size.y; j++) {
      int n = 2 * m_size.x + 1;
      triangles.emplace_back(i + j * n, i + 1 + j * n,
                             i + (j + 1) * n);
