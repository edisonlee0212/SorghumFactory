...
bool RayTracer::RenderDefault(const DefaultRenderingProperties &properties) {
  ...
  if (!m_defaultRenderingPipeline.m_accumulate ||
...
#pragma region Bind output texture as cudaSurface
  ...
  CUDA_CHECK(CreateSurfaceObject(
      &m_defaultRenderingLaunchParams.m_frame.m_outputTexture,
      &cudaResourceDesc));
#pragma endregion
...
#pragma region Remove texture binding.
  CUDA_CHECK(DestroySurfaceObject(
      m_defaultRenderingLaunchParams.m_frame.m_outputTexture));
...
__global__ void ApplyTransformKernel(int size, glm::mat4 globalTransform,
                                     Vertex *vertices,
  ...
  if (idx < size) {
    ...
    // targetNormals[idx] = glm::normalize(globalTransform *
    // glm::vec4(vertices[idx].m_normal, 0.0f)); targetTangents[idx] =
    // glm::normalize(globalTransform * glm::vec4(vertices[idx].m_tangent,
    // 0.0f));
  }
}
...
void RayTracer::BuildAccelerationStructure() {
  ...
  if (m_verticesBuffer.size() != m_instances.size())
    uploadVertices = true;
  else {
    ...
    if (uploadVertices || triangleMesh.m_transformUpdateFlag) {
      ...
      ApplyTransformKernel<<<gridSize, blockSize>>>(
          ...
          static_cast<glm::vec3 *>(
              m_transformedPositionsBuffer[meshID].m_dPtr));
      CUDA_SYNC_CHECK();
    }
  ...
  OPTIX_CHECK(optixAccelBuild(
      ...
      /* stream */ nullptr, &accelerateOptions, triangleInput.data(),
      static_cast<int>(m_instances.size()), tempBuffer.DevicePointer(),
    ...
    std::vector<cudaGraphicsResource_t> &boundResources) {
  {
    ...
    for (int i = 0; i < numObjects; i++) {
      for (int rayID = 0;
        ...
        rec.m_data.m_mesh.m_vertices =
            reinterpret_cast<Vertex *>(m_verticesBuffer[i].DevicePointer());
        ...
        rec.m_data.m_material.m_diffuseIntensity =
            m_instances[i].m_diffuseIntensity;
        rec.m_data.m_enableMLVQ = m_instances[i].m_enableMLVQ;
        if (m_instances[i].m_enableMLVQ) {
          rec.m_data.m_rayMlvqMaterial.m_btf =
              m_btfs[m_instances[i].m_mlvqMaterialIndex];
        }
...
#pragma region Bind output texture
            ...
            cudaTextureDesc.normalizedCoords = 1;
            CUDA_CHECK(CreateTextureObject(
                &rec.m_data.m_material.m_albedoTexture, &cudaResourceDesc,
...
#pragma region Bind output texture
            ...
            cudaGraphicsResource_t graphicsResource;
            CUDA_CHECK(GraphicsGLRegisterImage(
                &graphicsResource, m_instances[i].m_normalTexture,
                GL_TEXTURE_2D, cudaGraphicsRegisterFlagsReadOnly));
            CUDA_CHECK(GraphicsMapResources(1, &graphicsResource, nullptr));
            CUDA_CHECK(GraphicsSubResourceGetMappedArray(
                &textureArray, graphicsResource, 0, 0));
            ...
            cudaTextureDesc.normalizedCoords = 1;
            CUDA_CHECK(CreateTextureObject(
                &rec.m_data.m_material.m_normalTexture, &cudaResourceDesc,
                &cudaTextureDesc, nullptr));
#pragma endregion
            boundResources.push_back(graphicsResource);
