...
#pragma endregion
			...
			if (foliageEntity.HasPrivateComponent<Particles>())
			{
				...
				for (auto& matrix : matrices)
				{
					for (auto triangle : triangles)
					{
						leafIndices += "f " + std::to_string(triangle.x + offset + branchVerticesSize + branchletVerticesSize + 1)
							+ " " + std::to_string(triangle.y + offset + branchVerticesSize + branchletVerticesSize + 1)
