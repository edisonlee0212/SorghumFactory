    ...
    for (int xi = 0; xi < m_size.x; xi++) {
        for (int yi = 0; yi < m_size.y; yi++) {
            ...
            matricesList[selectedIndex].push_back(
                    glm::translate(glm::vec3(xi * m_distances.x - center.x, 0.0f, yi * m_distances.y - center.y)) *
...
void SorghumManager::Init() {
    ...
    sorghumManager.m_leafNodeMaterial = ResourceManager::LoadMaterial(false,
    ...
    sorghumManager.m_leafQuery.SetAllFilters(LeafInfo());
    ...
    const auto textureLeaf = ResourceManager::LoadTexture(false, FileIO::GetAssetFolderPath() +
                                                                 "Textures/leafSurfaceBright.jpg");
    sorghumManager.m_leafSurfaceTexture = ResourceManager::LoadTexture(false, FileIO::GetAssetFolderPath() +
                                                                              "Textures/leafSurfaceBright.jpg");
    ...
    sorghumManager.m_instancedLeafMaterial = ResourceManager::LoadMaterial(false,
    ...
    PlantManager::GetInstance().m_plantGrowthModels.insert_or_assign(PlantType::Sorghum, [](PlantManager &manager,
    ...
    PlantManager::GetInstance().m_plantMetaDataCalculators.insert_or_assign(PlantType::Sorghum,
...
void SorghumManager::GenerateMeshForAllSorghums(int segmentAmount, int step) {
    std::mutex meshMutex;
    EntityManager::ForEach<GlobalTransform>(JobManager::PrimaryWorkers(), GetInstance().m_leafQuery,
                                            [&meshMutex, segmentAmount, step]
                                                ...
                                                if (spline.m_startingPoint != -1) {
                                                    ...
                                                    for (float i = 0.0f;
                                                         i < spline.m_startingPoint - 0.05f; i += 0.05f) {
                                                    ...
                                                    for (float i = 0.05f; i <= 1.0f; i += 0.05f) {
                                                        ...
                                                        spline.m_nodes.emplace_back(spline.EvaluatePointFromCurve(i),
                                                                                    ...
                                                                                    spline.EvaluateAxisFromCurve(i),
                                                                                    true);
                                                    }
                                                } else {
                                                ...
                                                float rightFlatness = glm::gaussRand(1.75f,
                                                                                     0.5f);//glm::linearRand(0.5f, 2.0f);
                                                float leftFlatnessFactor = glm::gaussRand(1.25f,
                                                                                          0.2f);//glm::linearRand(1.0f, 2.5f);
                                                float rightFlatnessFactor = glm::gaussRand(1.25f,
                                                                                           0.2f);//glm::linearRand(1.0f, 2.5f);
                                                ...
                                                for (int i = 1; i < spline.m_nodes.size(); i++) {
                                                    ...
                                                    BezierCurve curve = BezierCurve(prev.m_position, prev.m_position +
                                                                                                     distance / 5.0f *
                                                                                                     prev.m_axis,
                                                                                    curr.m_position -
                                                                                    distance / 5.0f * curr.m_axis,
                                                                                    curr.m_position);
                                                    for (float div = 1.0f / segmentAmount;
                                                         div <= 1.0f; div += 1.0f / segmentAmount) {
                                                ...
                                                const float yLeafStep = 0.5f / (spline.m_segments.size() -
                                                                                static_cast<float>(stemSegmentCount) +
                                                                                1);
                                                for (int i = 0; i < spline.m_segments.size(); i++) {
                                                    ...
                                                    for (int j = 0; j < vertsCount; j++) {
                                                        ...
                                                        archetype.m_position = glm::vec3(position.x, position.y,
                                                                                         position.z);
                                                        float yPos = (i < stemSegmentCount) ? yStemStep * i : 0.5f +
                                                                                                              yLeafStep *
                                                                                                              (i -
                                                                                                               stemSegmentCount +
                                                                                                               1);
                                                    ...
                                                    if (i != 0) {
                                                        for (int j = 0; j < vertsCount - 1; j++) {
                                                            ...
                                                            spline.m_indices.push_back(
                                                                    vertexIndex + ((i - 1) + 1) * vertsCount + j);
                                                            spline.m_indices.push_back(
                                                                    vertexIndex + ((i - 1) + 1) * vertsCount + j + 1);
                                                        }
                                                    }
                                                }
    ...
    for (auto &plant : plants) {
        if (plant.GetDataComponent<PlantInfo>().m_plantType != PlantType::Sorghum) continue;
...
Entity SorghumManager::ImportPlant(const std::string &path, const std::string &name) {
    ...
    truckSpline.m_left = glm::cross(glm::vec3(0.0f, 1.0f, 0.0f),
                                    truckSpline.m_curves.begin()->m_p0 - truckSpline.m_curves.back().m_p3);
    for (int i = 0; i < leafCount; i++) {
        ...
        leafSpline.m_left = glm::cross(glm::vec3(0.0f, 1.0f, 0.0f),
                                       leafSpline.m_curves.begin()->m_p0 - leafSpline.m_curves.back().m_p3);
    }
    return sorghum;
}
    ...
    if (ImGui::BeginMainMenuBar()) {
        if (ImGui::BeginMenu("Sorghum Manager")) {
            ...
            if (ImGui::BeginPopupModal("Sorghum field wizard", nullptr, ImGuiWindowFlags_AlwaysAutoResize)) {
                ...
                if (ImGui::Button("OK", ImVec2(120, 0))) {
                    ...
                    candidates.push_back(ImportPlant(FileIO::GetAssetFolderPath() + "Sorghum/skeleton_procedural_1.txt",
                                                     "Sorghum 1"));
                    candidates.push_back(ImportPlant(FileIO::GetAssetFolderPath() + "Sorghum/skeleton_procedural_2.txt",
                                                     "Sorghum 2"));
                    candidates.push_back(ImportPlant(FileIO::GetAssetFolderPath() + "Sorghum/skeleton_procedural_3.txt",
                                                     "Sorghum 3"));
                    candidates.push_back(ImportPlant(FileIO::GetAssetFolderPath() + "Sorghum/skeleton_procedural_4.txt",
                                                     "Sorghum 4"));
            ...
            if (ImGui::Button("Lock structure")) {
                ...
                PlantManager::GetInstance().m_plantQuery.ToEntityArray<PlantInfo>(sorghums, [](const Entity &plant,
        ...
        if (ImGui::BeginPopupModal("Illumination Estimation", nullptr, ImGuiWindowFlags_AlwaysAutoResize)) {
            ...
            std::string text = std::to_string(static_cast<int>(fraction * 100.0f)) + "% - " +
                               std::to_string(manager.m_processingEntities.size() - manager.m_processingIndex) + "/" +
                               std::to_string(manager.m_processingEntities.size());
            ImGui::ProgressBar(fraction, ImVec2(240, 0), text.c_str());
            ImGui::SetItemDefaultFocus();
            ImGui::Text(("Estimation time for 1 plant: " + std::to_string(manager.m_perPlantCalculationTime) +
                         " seconds").c_str());
...
void SorghumManager::CloneSorghums(const Entity &parent, const Entity &original, std::vector<glm::mat4> &matrices) {
    for (const auto &matrix : matrices) {
        ...
        original.ForEachChild([&sorghum, &matrices](Entity child) {
            if (!child.HasDataComponent<LeafInfo>()) return;
...
void SorghumManager::ExportSorghum(const Entity &sorghum, std::ofstream &of, unsigned &startIndex) {
    ...
    of.flush();
    const auto position = sorghum.GetDataComponent<GlobalTransform>().GetPosition();
    sorghum.ForEachChild([&](Entity child) {
...
void SorghumManager::ObjExportHelper(glm::vec3 position, std::shared_ptr<Mesh> mesh,
    ...
    if (!mesh->UnsafeGetTriangles().empty()) {
        std::string header = "#Vertices: " + std::to_string(mesh->GetVerticesAmount()) + ", tris: " +
                             std::to_string(mesh->GetTriangleAmount());
        header += "\n";
...
void SorghumManager::RenderLightProbes() {
    auto &manager = GetInstance();
    if (manager.m_probeTransforms.empty() || manager.m_probeColors.empty() ||
...
#pragma endregion
        ...
        while (!walker.IsNull()) {
            Entity temp;
            walker.ForEachChild([&](Entity child) {
                                    if (child.HasDataComponent<InternodeInfo>() && child.GetDataComponent<InternodeInfo>().m_order == 1) {
                                        temp = child;
        ...
        glm::vec3 startPosition = sorghum.GetDataComponent<GlobalTransform>().GetPosition();
        glm::vec3 endPosition = centerNode.back().GetDataComponent<GlobalTransform>().GetPosition();
        auto truckCurve = BezierCurve(glm::vec3(0.0f), (-startPosition + endPosition) / 2.0f,
        ...
        for (int i = 0; i < centerNode.size(); i++) {
            glm::vec3 centerNodePosition = centerNode[i].GetDataComponent<GlobalTransform>().GetPosition();
            centerNode[i].ForEachChild([&](Entity child) {
                                           if (child.HasDataComponent<InternodeInfo>() && child.GetDataComponent<InternodeInfo>().m_order == 2) {
                                               ...
                                               auto leafCurve = BezierCurve(cp0 - startPosition, cp1 - startPosition, cp2 - startPosition,
                                                                            cp3 - startPosition);
                                               ...
                                               leafCurve = BezierCurve(cp0 - startPosition, cp1 - startPosition, cp2 - startPosition,
                                                                       cp3 - startPosition);
                                               leafSpline.m_curves.push_back(leafCurve);
...
void SorghumManager::FormCandidates(PlantManager &manager,
    ...
    EntityManager::ForEach<GlobalTransform, Transform, InternodeInfo, InternodeGrowth, InternodeStatistics, Illumination>(
            ...
            [&, globalTime, sphereSize](int index, Entity internode, GlobalTransform &globalTransform,
                ...
                if (internode.GetChildrenAmount() != 0) return;
                ...
                if (internodeInfo.m_order == 1) {
                    ...
                    stemCandidate.m_growth.m_internodeLength = glm::gaussRand(parameters.m_internodeLength,
                                                                              parameters.m_internodeLengthVariance);
                    stemCandidate.m_growth.m_distanceToRoot = internodeGrowth.m_distanceToRoot + 1;
                    float rotateAngle = glm::gaussRand(180.0f, parameters.m_rollAngleVariance +
...
#pragma region Calculate transform
                    glm::quat globalRotation =
                            globalTransform.GetRotation() * stemCandidate.m_growth.m_desiredLocalRotation;
                    ...
                    stemCandidate.m_transform.m_value =
                            glm::inverse(globalTransform.m_value) * stemCandidate.m_globalTransform.m_value;
#pragma endregion
...
#pragma region Calculate transform
                        glm::quat globalRotation =
                                globalTransform.GetRotation() * leafCandidate.m_growth.m_desiredLocalRotation;
                        glm::vec3 front = globalRotation * glm::vec3(0, 0, -1);
                        ...
                        leafCandidate.m_transform.m_value =
                                glm::inverse(globalTransform.m_value) * leafCandidate.m_globalTransform.m_value;
#pragma endregion
...
void SorghumManager::FormLeafNodes(PlantManager &plantManager) {
    ...
    for (auto &i : candidates) {
        if (i.first.GetChildrenAmount() != 0)EntityManager::DeleteEntity(i.first.GetChildren()[0]);
        ...
        meshRenderer2.m_material = GetInstance().m_leafNodeMaterial;
        ...
        meshRenderer3.m_material = GetInstance().m_leafNodeMaterial;
        ...
        transform.SetEulerRotation(
                glm::radians(glm::vec3(0, leafGravityBending + 2.0 * leafGravityBendingIncrease, 0)));
        ...
        meshRenderer4.m_material = GetInstance().m_leafNodeMaterial;
        ...
        transform.SetPosition(glm::vec3(0, 0, -leafNodeDistance));
        transform.SetEulerRotation(
                glm::radians(glm::vec3(0, leafGravityBending + 3.0 * leafGravityBendingIncrease, 0)));
        ...
        meshRenderer5.m_material = GetInstance().m_leafNodeMaterial;
        ...
        transform.SetPosition(glm::vec3(0, 0, -leafNodeDistance));
        transform.SetEulerRotation(
                glm::radians(glm::vec3(0, leafGravityBending + 4.0 * leafGravityBendingIncrease, 0)));
        ...
        meshRenderer6.m_material = GetInstance().m_leafNodeMaterial;
        ...
        transform.SetPosition(glm::vec3(0, 0, -leafNodeDistance));
        transform.SetEulerRotation(
                glm::radians(glm::vec3(0, leafGravityBending + 5.0 * leafGravityBendingIncrease, 0)));
        ...
        meshRenderer8.m_material = GetInstance().m_leafNodeMaterial;
...
void SorghumManager::Update() {
    ...
    if (manager.m_processing) {
        ...
        if (manager.m_processingIndex == -1) {
            ...
            manager.m_probeTransforms.insert(manager.m_probeTransforms.end(), estimator.m_probeTransforms.begin(),
                                             estimator.m_probeTransforms.end());
            manager.m_probeColors.insert(manager.m_probeColors.end(), estimator.m_probeColors.begin(),
            ...
            manager.m_lightProbeRenderingColorBuffer.SetData(static_cast<GLsizei>(count) * sizeof(glm::vec4),
                                                             manager.m_probeColors.data(), GL_DYNAMIC_DRAW);
            manager.m_lightProbeRenderingTransformBuffer.SetData(static_cast<GLsizei>(count) * sizeof(glm::mat4),
                                                                 manager.m_probeTransforms.data(), GL_DYNAMIC_DRAW);
        }
    }
}
