        ...
        for (int levelStep = 0; levelStep < totalLevelStep; levelStep++) {
            ...
            for (int sliceIndex = 0; sliceIndex < m_sectorAmount; sliceIndex++) {
                for (int angleStep = 0; angleStep < totalAngleStep; angleStep++) {
                    ...
                    glm::vec3 position =
                            glm::normalize(glm::vec3(x, 0.0f, z)) * m_cakeTiers[tierIndex][sliceIndex].m_maxDistance;
                    position.y = currentHeight;
                    vertices[levelStep * totalAngleStep * m_sectorAmount + actualAngleStep].m_position = position;
                    vertices[levelStep * totalAngleStep * m_sectorAmount + actualAngleStep].m_texCoords = glm::vec2(
                            (float) levelStep / (totalLevelStep - 1), (float) angleStep / (totalAngleStep - 1));
                    vertices[levelStep * totalAngleStep * m_sectorAmount + actualAngleStep].m_normal = glm::normalize(
                            position);
                    vertices[totalLevelStep * m_sectorAmount * totalAngleStep +
                             levelStep * totalAngleStep * m_sectorAmount + actualAngleStep].m_position = position;
                    vertices[totalLevelStep * m_sectorAmount * totalAngleStep +
                             levelStep * totalAngleStep * m_sectorAmount + actualAngleStep].m_texCoords = glm::vec2(
                            (float) levelStep / (totalLevelStep - 1), (float) angleStep / (totalAngleStep - 1));
                    vertices[totalLevelStep * m_sectorAmount * totalAngleStep +
                             levelStep * totalAngleStep * m_sectorAmount + actualAngleStep].m_normal = glm::vec3(0,
                                                                                                                 levelStep ==
                                                                                                                 0 ? -1
                                                                                                                   : 1,
                                                                                                                 0);
                ...
                for (int angleStep = 0; angleStep < totalAngleStep; angleStep++) {
                    ...
                    if (actualAngleStep < m_sectorAmount * totalAngleStep - 1) {
                        ...
                        indices[12 * (levelStep * totalAngleStep * m_sectorAmount + actualAngleStep) + 6] =
                                totalLevelStep * m_sectorAmount * totalAngleStep +
                                levelStep * totalAngleStep * m_sectorAmount + actualAngleStep;
...
void RadialBoundingVolume::CalculateVolume() {
    ...
    internodeDataQuery.ToComponentDataArray<InternodeInfo, InternodeInfo>(internodeInfos,
                                                                          [=](const InternodeInfo &info) {
    ...
    for (auto &internode : internodes) {
        ...
        for (int i = 0; i < segments; i++) {
            const glm::vec3 position = positions[positionIndex] +
                                       (parentNodePosition - positions[positionIndex]) * (float) i / (float) segments;
            ...
            if (currentDistance <= internodeGrowth.m_thickness) {
                for (auto &slice : m_cakeTiers[sliceIndex.x]) {
                    if (slice.m_maxDistance < currentDistance + internodeGrowth.m_thickness)
                        slice.m_maxDistance = currentDistance + internodeGrowth.m_thickness;
                }
            } else if (m_cakeTiers[sliceIndex.x][sliceIndex.y].m_maxDistance < currentDistance)
...
void RadialBoundingVolume::OnGui() {
    ...
    if (m_display && m_meshGenerated) {
        for (auto &i : m_boundMeshes) {
            RenderManager::DrawGizmoMesh(i.get(), EditorManager::GetSceneCamera(), m_displayColor,
...
bool RadialBoundingVolume::InVolume(const glm::vec3 &position) {
    ...
    if (m_meshGenerated) {
        const auto sliceIndex = SelectSlice(position);
        const float currentDistance = glm::length(glm::vec2(position.x, position.z));
        return glm::max(1.0f, m_cakeTiers[sliceIndex.x][sliceIndex.y].m_maxDistance) >= currentDistance &&
