            ...
            for (int k = -1; k <= 1; k++) {
                ...
                for (int i = 0; i < voxel.m_positions.size(); i++) {
                    ...
                    if (owner != voxel.m_owners[i] &&
                        vector.x * vector.x + vector.y * vector.y + vector.z * vector.z < radius * radius)
                        return true;
                }
            }
        }
...
std::pair<bool, bool>
VoxelSpace::HasObstacleConeSphere(const float &angle, const glm::vec3 &position, const glm::vec3 &direction,
        ...
        for (int j = -1; j <= 1; j++) {
            for (int k = -1; k <= 1; k++) {
                ...
                for (int p = 0; p < voxel.m_positions.size(); p++) {
                    const float dot = glm::dot(glm::normalize(direction),
                    ...
                    if (owner == voxel.m_owners[p] && distance2 < selfRadius * selfRadius && dot > cosAngle)
                        return {true, false};
...
bool VoxelSpace::HasObstacleConeSameOwner(const float &angle, const glm::vec3 &position,
                                          const glm::vec3 &direction, const Entity &owner, const Entity &internode,
                                          const Entity &parent, float selfRadius) {
    std::vector<Voxel *> checkedVoxel;
    for (int i = -1; i <= 1; i++) {
        for (int j = -1; j <= 1; j++) {
            for (int k = -1; k <= 1; k++) {
                ...
                for (int p = 0; p < voxel.m_positions.size(); p++) {
                    ...
                    if (parent != compare && internode != compare && owner == voxel.m_owners[p] &&
