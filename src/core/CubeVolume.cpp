...
void CubeVolume::ApplyMeshRendererBounds() {
    ...
    m_minMaxBound.m_min =
            globalTransform.GetPosition() + glm::vec3(glm::vec4(bound.m_min, 1.0f) * globalTransform.m_value);
    m_minMaxBound.m_max =
            globalTransform.GetPosition() + glm::vec3(glm::vec4(bound.m_max, 1.0f) * globalTransform.m_value);
}
...
void CubeVolume::OnGui() {
    ...
    if (m_displayBounds) {
        RenderManager::DrawGizmoMesh(DefaultResources::Primitives::Cube.get(), *RenderManager::GetMainCamera(),
                                     glm::vec4(0, 1, 0, 0.2),
                                     glm::translate(m_minMaxBound.Center()) * glm::scale(m_minMaxBound.Size()), 1);
    }
