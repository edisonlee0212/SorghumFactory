...
void TreeManager::WriteChain(int order, Entity internode, rapidxml::xml_node<> *chains,
    ...
    chain->append_attribute(
            doc->allocate_attribute("gravelius", doc->allocate_string(std::to_string(order + 1).c_str())));
    ...
    while (EntityManager::GetChildrenAmount(walker) != 0) {
        ...
        node->append_attribute(
                doc->allocate_attribute("id", doc->allocate_string(std::to_string(walker.m_index).c_str())));
    ...
    if (EntityManager::GetParent(internode).HasComponentData<InternodeInfo>()) {
        ...
        node->append_attribute(doc->allocate_attribute("id", doc->allocate_string(
                std::to_string(EntityManager::GetParent(internode).m_index).c_str())));
        chain->append_node(node);
    }
}
...
void TreeManager::UpdateBranchCylinder(const bool &displayThickness, const float &width) {
    ...
    EntityManager::ForEach<GlobalTransform, BranchCylinder, BranchCylinderWidth, InternodeGrowth, InternodeInfo>(
            JobManager::PrimaryWorkers(), plantManager.m_internodeQuery, [displayThickness, width]
                    (int i, Entity entity, GlobalTransform &ltw, BranchCylinder &c,
                     BranchCylinderWidth &branchCylinderWidth, InternodeGrowth &internodeGrowth,
                ...
                const glm::vec3 parentTranslation = EntityManager::GetParent(
                ...
                if (EntityManager::HasPrivateComponent<TreeData>(EntityManager::GetParent(entity))) {
                    c.m_value = glm::translate((translation + parentTranslation) / 2.0f) * rotationTransform *
                    ...
                    c.m_value = glm::translate((translation + parentTranslation) / 2.0f) * rotationTransform *
                                glm::scale(glm::vec3(branchCylinderWidth.m_value,
...
void TreeManager::UpdateBranchPointer(const float &length, const float &width) {
    ...
    switch (manager.m_pointerRenderType) {
        case PointerRenderType::Illumination:
            EntityManager::ForEach<GlobalTransform, BranchPointer, Illumination>(JobManager::PrimaryWorkers(),
                                                                                 ...
                                                                                 [length, width]
                                                                                     ...
                                                                                     const glm::vec3 direction = glm::normalize(
                                                                                             internodeIllumination.m_accumulatedDirection);
                                                                                     const glm::vec3 end = start +
                                                                                                           direction *
                                                                                                           length *
                                                                                                           glm::length(
                                                                                                                   internodeIllumination.m_accumulatedDirection);
                                                                                     const glm::quat rotation = glm::quatLookAt(
                                                                                             direction,
                                                                                             glm::vec3(0, 0, 1));
        ...
        case PointerRenderType::Bending:
            EntityManager::ForEach<GlobalTransform, BranchPointer, InternodeGrowth>(JobManager::PrimaryWorkers(),
                                                                                    plantManager.m_internodeQuery,
                                                                                    [length, width]
                                                                                            (int i, Entity entity,
                                                                                             GlobalTransform &ltw,
...
#pragma region Process internode color
    switch (GetInstance().m_branchRenderType) {
        case BranchRenderType::Illumination: {
            EntityManager::ForEach<BranchColor, Illumination, InternodeInfo>(JobManager::PrimaryWorkers(),
                                                                             plantManager.m_internodeQuery, [=]
                                                                                     (int i, Entity entity,
        ...
        case BranchRenderType::Inhibitor: {
            EntityManager::ForEach<BranchColor, InternodeGrowth>(JobManager::PrimaryWorkers(),
                                                                 plantManager.m_internodeQuery, [=]
                                                                         (int i, Entity entity,
                                                                          BranchColor &internodeRenderColor,
                                                                          InternodeGrowth &internodeGrowth) {
        ...
        case BranchRenderType::Order: {
            EntityManager::ForEach<BranchColor, InternodeInfo>(JobManager::PrimaryWorkers(),
                                                               plantManager.m_internodeQuery, [=]
                                                                       (int i, Entity entity,
                                                                        BranchColor &internodeRenderColor,
                                                                        InternodeInfo &internodeInfo) {
        ...
        case BranchRenderType::MaxChildOrder: {
            EntityManager::ForEach<BranchColor, InternodeStatistics>(JobManager::PrimaryWorkers(),
                                                                     plantManager.m_internodeQuery, [=]
                                                                             (int i, Entity entity,
                                                                              BranchColor &internodeRenderColor,
...
Entity TreeManager::CreateTree(const Transform &transform) {
    ...
    const auto rootInternode = EntityManager::GetChildren(plant)[0];
        ...
        if (ImGui::BeginMenu("Tree Manager")) {
            ...
            if (ImGui::BeginPopupModal("New tree wizard", nullptr, ImGuiWindowFlags_AlwaysAutoResize)) {
                ...
                for (auto i = 0; i < newTreeAmount; i++) {
                    ...
                    const bool opened = ImGui::TreeNodeEx(title.c_str(), ImGuiTreeNodeFlags_NoTreePushOnOpen |
...
#pragma region Menu
                    ...
                    if (ImGui::ButtonEx("To present", ImVec2(0, 0),
                                        manager.m_displayTime != plantManager.m_globalTime ? 0
                                                                                           : ImGuiButtonFlags_Disabled))
                        manager.m_displayTime = plantManager.m_globalTime;
                    ...
                    if (manager.m_drawBranches) {
                        if (ImGui::TreeNodeEx("Connection settings", ImGuiTreeNodeFlags_DefaultOpen)) {
                            ImGui::Combo("Render type", (int *) &manager.m_branchRenderType, BranchRenderTypes,
                                         IM_ARRAYSIZE(BranchRenderTypes));
                            ...
                            if (manager.m_enableBranchDataCompress)
                                ImGui::DragFloat("Compress factor", &manager.m_branchCompressFactor, 0.01f, 0.01f,
                                                 1.0f);
                            ...
                            if (manager.m_useColorMap) {
                                ...
                                if (savedAmount != manager.m_colorMapSegmentAmount) {
                                    ...
                                    for (int i = 0; i < manager.m_colorMapSegmentAmount; i++) {
                                        if (i != 0 &&
                                            manager.m_colorMapValues[i] < manager.m_colorMapValues[i - 1])
                                            manager.m_colorMapValues[i] = manager.m_colorMapValues[i - 1] + 1;
                                        if (i >= savedAmount)
                                            manager.m_colorMapColors[i] = glm::linearRand(glm::vec3(0.0f),
                                                                                          glm::vec3(1.0f));
                                    }
                                ...
                                for (int i = 0; i < manager.m_colorMapValues.size(); i++) {
                                    if (i == 0) {
                                        ImGui::DragFloat("Value 0", &manager.m_colorMapValues[0], 0.1f, 0.0f,
                                                         manager.m_colorMapValues[1]);
                                        ...
                                        ImGui::DragFloat(("Value" + std::to_string(i)).c_str(),
                                                         &manager.m_colorMapValues[i], 0.1f,
                                                         manager.m_colorMapValues[i - 1] + 0.1f, 9999.0f);
                                        ImGui::ColorEdit3(("Color" + std::to_string(i)).c_str(),
                                                          &manager.m_colorMapColors[i].x);
                                    } else {
                                        ImGui::DragFloat(("Value" + std::to_string(i)).c_str(),
                                                         &manager.m_colorMapValues[i], 0.1f,
                                                         manager.m_colorMapValues[i - 1] + 0.1f,
                                                         manager.m_colorMapValues[i + 1]);
                                        ImGui::ColorEdit3(("Color" + std::to_string(i)).c_str(),
                                                          &manager.m_colorMapColors[i].x);
                                    }
                                }
                            ...
                            if (ImGui::Checkbox("Display thickness", &manager.m_displayThickness))
                                manager.m_updateBranch = true;
                            if (!manager.m_displayThickness)
                                if (ImGui::DragFloat("Connection width", &manager.m_connectionWidth, 0.01f, 0.01f,
                                                     1.0f))
                                    manager.m_updateBranch = true;
                            ImGui::TreePop();
                        }
                    ...
                    if (manager.m_drawPointers) {
                        if (ImGui::TreeNodeEx("Pointer settings", ImGuiTreeNodeFlags_DefaultOpen)) {
                            ImGui::Combo("Render type", (int *) &manager.m_pointerRenderType, PointerRenderTypes,
                                         IM_ARRAYSIZE(PointerRenderTypes));
                            ImGui::Checkbox("Compress", &manager.m_enablePointerDataCompress);
                            if (manager.m_pointerCompressFactor)
                                ImGui::DragFloat("Compress factor", &manager.m_branchCompressFactor, 0.01f, 0.01f,
                                                 1.0f);
                            if (ImGui::ColorEdit4("Pointer color", &manager.m_pointerColor.x))
                                manager.m_updatePointer = true;
                            if (ImGui::DragFloat("Pointer length", &manager.m_pointerLength, 0.01f, 0.01f, 3.0f))
                                manager.m_updatePointer = true;
                            if (ImGui::DragFloat("Pointer width", &manager.m_pointerWidth, 0.01f, 0.01f, 1.0f))
                                manager.m_updatePointer = true;
                            ImGui::TreePop();
                        }
                    }
...
#pragma endregion
            ...
            ImGui::Image(
                    reinterpret_cast<ImTextureID>(manager.m_internodeDebuggingCamera->GetTexture()->Texture()->Id()),
...
#pragma region Ray selection
                        ...
                        cameraLtw.m_value = glm::translate(EditorManager::GetInstance().m_sceneCameraPosition) *
...
void TreeManager::RenderBranchCylinders(const float &displayTime) {
    ...
    auto &plantManager = PlantManager::GetInstance();
    auto &manager = GetInstance();
    plantManager.m_internodeQuery.ToComponentDataArray<BranchCylinder, InternodeInfo>(branchCylinders, [displayTime](
            const InternodeInfo &internodeInfo) {
        return internodeInfo.m_startGlobalTime <= displayTime;
    });
    std::vector<BranchColor> branchColors;
    plantManager.m_internodeQuery.ToComponentDataArray<BranchColor, InternodeInfo>(branchColors, [displayTime](
            const InternodeInfo &internodeInfo) {
        return internodeInfo.m_startGlobalTime <= displayTime;
    });
...
void TreeManager::RenderBranchPointers(const float &displayTime) {
    ...
    auto &plantManager = PlantManager::GetInstance();
    auto &manager = GetInstance();
    plantManager.m_internodeQuery.ToComponentDataArray<BranchPointer, InternodeInfo>(branchPointers, [displayTime](
            const InternodeInfo &internodeInfo) {
        return internodeInfo.m_startGlobalTime <= displayTime;
    });
...
void
TreeManager::TreeNodeWalker(std::vector<Entity> &boundEntities, std::vector<int> &parentIndices, const int &parentIndex,
                            const Entity &node) {
...
void TreeManager::FormCandidates(PlantManager &manager,
    ...
    std::mutex mutex;
    EntityManager::ForEach<GlobalTransform, Transform, InternodeInfo, InternodeGrowth, InternodeStatistics, Illumination>(
            JobManager::PrimaryWorkers(), PlantManager::GetInstance().m_internodeQuery,
            [&, globalTime](int index, Entity internode, GlobalTransform &globalTransform, Transform &transform,
...
#pragma region Form candidate
                    ...
                    candidate.m_growth.m_inhibitorTransmitFactor = GetGrowthParameter(
                            GrowthParameterType::InhibitorTransmitFactor, treeData, internodeInfo, internodeGrowth,
                            internodeStatistics);
                    ...
                    if (!bud.m_isApical) {
                        desiredRotation = glm::rotate(desiredRotation,
                                                      GetGrowthParameter(GrowthParameterType::BranchingAngle, treeData,
...
#pragma region Apply tropisms
                    ...
                    PlantManager::ApplyTropism(-treeData->m_gravityDirection,
                    ...
                    if (internodeIllumination.m_accumulatedDirection != glm::vec3(0.0f))
                        PlantManager::ApplyTropism(glm::normalize(internodeIllumination.m_accumulatedDirection),
                                                   GetGrowthParameter(GrowthParameterType::Phototropism, treeData,
                                                                      internodeInfo, internodeGrowth,
                                                                      internodeStatistics), desiredFront, desiredUp);
...
#pragma region Calculate transform
                    glm::quat globalRotation =
                            globalTransform.GetRotation() * candidate.m_growth.m_desiredLocalRotation;
                    ...
                    candidate.m_transform.m_value =
                            glm::inverse(globalTransform.m_value) * candidate.m_globalTransform.m_value;
#pragma endregion
                    ...
                    apicalBud.m_resourceWeight = GetGrowthParameter(GrowthParameterType::ResourceWeightApical, treeData,
                                                                    internodeInfo, internodeGrowth,
                                                                    internodeStatistics);
                    totalResourceWeight += apicalBud.m_resourceWeight;
                    apicalBud.m_avoidanceAngle = GetGrowthParameter(GrowthParameterType::AvoidanceAngle, treeData,
                                                                    internodeInfo, internodeGrowth,
                                                                    internodeStatistics);
                    candidate.m_buds.push_back(apicalBud);
                    const auto budAmount = GetGrowthParameter(GrowthParameterType::LateralBudPerNode, treeData,
                                                              internodeInfo, internodeGrowth, internodeStatistics);
                    for (int budIndex = 0; budIndex < budAmount; budIndex++) {
                        Bud lateralBud;
                        lateralBud.m_isApical = false;
                        lateralBud.m_resourceWeight = GetGrowthParameter(GrowthParameterType::ResourceWeightLateral,
                        ...
                        lateralBud.m_avoidanceAngle = GetGrowthParameter(GrowthParameterType::AvoidanceAngle, treeData,
                                                                         internodeInfo, internodeGrowth,
                                                                         internodeStatistics);
...
float TreeManager::GetGrowthParameter(const GrowthParameterType &type,
                                      std::unique_ptr<TreeData> &treeData, InternodeInfo &internodeInfo,
                                      InternodeGrowth &internodeGrowth,
                                      InternodeStatistics &internodeStatistics) {
    float value = 0;
    switch (type) {
        ...
        case GrowthParameterType::BranchingAngle:
            value = glm::radians(glm::gaussRand(treeData->m_parameters.m_branchingAngleMean,
                                                treeData->m_parameters.m_branchingAngleVariance));
            break;
        case GrowthParameterType::ApicalAngle:
            value = glm::radians(glm::gaussRand(treeData->m_parameters.m_apicalAngleMean,
                                                treeData->m_parameters.m_apicalAngleVariance));
            break;
        case GrowthParameterType::RollAngle:
            value = glm::radians(
                    glm::gaussRand(treeData->m_parameters.m_rollAngleMean, treeData->m_parameters.m_rollAngleVariance));
            break;
        ...
        case GrowthParameterType::ResourceWeightApical:
            value = treeData->m_parameters.m_resourceWeightApical *
                    (1.0f + glm::gaussRand(0.0f, treeData->m_parameters.m_resourceWeightVariance));
            break;
...
void TreeManager::PruneTrees(PlantManager &manager, std::vector<Volume *> &obstacles) {
    ...
    EntityManager::ForEach<GlobalTransform, InternodeInfo>(JobManager::PrimaryWorkers(), manager.m_internodeQuery,
                                                           [&](int index, Entity internode,
                                                               GlobalTransform &globalTransform,
                                                               InternodeInfo &internodeInfo) {
                                                               treeManager.m_voxelSpaceModule.Push(
    ...
    for (int i = 0; i < manager.m_plants.size(); i++) {
        if (manager.m_plants[i].HasPrivateComponent<TreeData>()) {
            ...
            distanceLimits[i] = 0;
            EntityManager::ForEachChild(manager.m_plants[i],
    ...
    std::mutex mutex;
    EntityManager::ForEach<GlobalTransform, InternodeInfo, Illumination, InternodeStatistics, InternodeGrowth>(
            JobManager::PrimaryWorkers(), manager.m_internodeQuery,
            [&](int index, Entity internode, GlobalTransform &globalTransform, InternodeInfo &internodeInfo,
                Illumination &illumination, InternodeStatistics &internodeStatistics,
                InternodeGrowth &internodeGrowth) {
                if (internodeInfo.m_plantType != PlantType::GeneralTree) return;
                int targetIndex = 0;
                ...
                if (angle > 0 && treeManager.m_voxelSpaceModule.HasObstacleConeSameOwner(angle,
                                                                                         globalTransform.GetPosition() -
                                                                                         direction *
                ...
                if (treeManager.m_voxelSpaceModule.HasNeighborFromDifferentOwner(globalTransform.GetPosition(),
                                                                                 internodeInfo.m_plant,
                                                                                 treeManager.m_crownShynessDiameter)) {
                    std::lock_guard lock(mutex);
                    cutOff.push_back(internode);
                    return;
                }
                const float randomCutOffProb = glm::min(manager.m_deltaTime * randomCutOffMaxes[targetIndex],
                                                        manager.m_deltaTime * randomCutOffs[targetIndex] +
                                                        (manager.m_globalTime - internodeInfo.m_startGlobalTime) *
...
void TreeManager::UpdateTreesMetaData(PlantManager &manager) {
    EntityManager::ForEach<PlantInfo, GlobalTransform>(JobManager::PrimaryWorkers(), manager.m_plantQuery,
...
#pragma endregion
        ...
        currentInternodeGrowth.m_childMeanPosition =
                currentInternodeGrowth.m_childrenTotalTorque / currentInternodeGrowth.m_MassOfChildren;
        const float strength = currentInternodeGrowth.m_MassOfChildren * glm::distance(
                glm::vec2(currentInternodeGrowth.m_childMeanPosition.x, currentInternodeGrowth.m_childMeanPosition.z),
        ...
        currentInternodeGrowth.m_sagging = glm::min(treeData->m_parameters.m_gravityBendingMax,
                                                    strength * treeData->m_parameters.m_gravityBendingFactor / glm::pow(
                                                            currentInternodeGrowth.m_thickness /
...
#pragma region Update self status
        ...
        const float strength = currentInternodeGrowth.m_MassOfChildren * glm::distance(
                ...
                glm::vec2(currentInternodeGrowth.m_desiredGlobalPosition.x,
                          currentInternodeGrowth.m_desiredGlobalPosition.z));
        currentInternodeGrowth.m_sagging = strength * treeData->m_parameters.m_gravityBendingFactor / glm::pow(
                currentInternodeGrowth.m_thickness / treeData->m_parameters.m_endNodeThickness,
                treeData->m_parameters.m_gravityBendingThicknessFactor);
        currentInternodeGrowth.m_thickestChild = child;
#pragma endregion
...
#pragma region Gravity bending.
        ...
        const glm::vec3 globalPosition =
                currentInternodeGlobalTransform.GetPosition() + front * treeData->m_parameters.m_internodeLengthBase;
...
void TreeManager::ResetTimeForTree(const float &value) {
    ...
    EntityManager::ForEach<InternodeInfo>(JobManager::PrimaryWorkers(), manager.m_internodeQuery,
...
void TreeManager::DistributeResourcesForTree(PlantManager &manager,
    ...
    EntityManager::ForEach<GlobalTransform, InternodeInfo, InternodeGrowth, InternodeStatistics>(
            JobManager::PrimaryWorkers(), manager.m_internodeQuery,
            [&](int index, Entity internode, GlobalTransform &globalTransform, InternodeInfo &internodeInfo,
                InternodeGrowth &internodeGrowth, InternodeStatistics &internodeStatistics) {
                if (internodeInfo.m_plantType != PlantType::GeneralTree) return;
                for (int i = 0; i < plants.size(); i++) {
                    if (plants[i] == internodeInfo.m_plant) {
                        const float internodeRequirement =
                                glm::max(heightResourceFactorMin[i], 1.0f - glm::pow(
                                        globalTransform.GetPosition().y - treePositions[i].y, heightResourceFactor[i]) *
    ...
    EntityManager::ForEach<GlobalTransform, Illumination, InternodeInfo, InternodeGrowth, InternodeStatistics>(
            JobManager::PrimaryWorkers(), manager.m_internodeQuery,
            [=, &manager](int index, Entity internode, GlobalTransform &globalTransform, Illumination &illumination,
                          InternodeInfo &internodeInfo, InternodeGrowth &internodeGrowth,
                          InternodeStatistics &internodeStatistics) {
                if (internodeInfo.m_plantType != PlantType::GeneralTree) return;
                for (int i = 0; i < plants.size(); i++) {
                    if (plants[i] == internodeInfo.m_plant) {
                        ...
                        const float internodeNutrient =
                                glm::min(totalNutrients[i].m_nutrient / (divisors[i] / requirementMaximums[i]),
                                         manager.m_deltaTime) * budsRequirement * internodeRequirement /
                                requirementMaximums[i];
                        ...
                        for (auto &bud : internodeData->m_buds) {
                            if (bud.m_active && !bud.m_enoughForGrowth) {
                                ResourceParcel resourceParcel = ResourceParcel(
                                        ...
                                        internodeCarbon / (bud.m_isApical ? apicalIlluminationRequirements[i]
...
#pragma region Materials
    for (int i = 0; i < 64; i++) {
        manager.m_randomColors.emplace_back(glm::linearRand(0.0f, 1.0f), glm::linearRand(0.0f, 1.0f),
                                            glm::linearRand(0.0f, 1.0f));
    }
    manager.m_defaultRayTracingBranchAlbedoTexture = ResourceManager::LoadTexture(false, FileIO::GetAssetFolderPath() +
                                                                                         "Textures/BarkMaterial/Bark_Pine_baseColor.jpg");
    manager.m_defaultRayTracingBranchNormalTexture = ResourceManager::LoadTexture(false, FileIO::GetAssetFolderPath() +
                                                                                         "Textures/BarkMaterial/Bark_Pine_normal.jpg");
    manager.m_defaultBranchAlbedoTexture = ResourceManager::LoadTexture(false, FileIO::GetAssetFolderPath() +
                                                                               "Textures/BarkMaterial/Bark_Pine_baseColor.jpg");
    manager.m_defaultBranchNormalTexture = ResourceManager::LoadTexture(false, FileIO::GetAssetFolderPath() +
                                                                               "Textures/BarkMaterial/Bark_Pine_normal.jpg");
#pragma endregion
#pragma region General tree growth
    ...
    plantManager.m_plantGrowthModels.insert_or_assign(PlantType::GeneralTree, [](PlantManager &manager,
                                                                                 std::vector<InternodeCandidate> &candidates) {
                                                          FormCandidates(manager, candidates);
                                                      }
    );

    plantManager.m_plantInternodePruners.insert_or_assign(PlantType::GeneralTree,
...
void TreeManager::Serialize(const Entity &treeEntity, rapidxml::xml_document<> &doc,
    ...
    PlantManager::GetInstance().m_internodeQuery.ToEntityArray<InternodeInfo>(internodes,
                                                                              [treeEntity](const Entity &entity,
                                                                                           const InternodeInfo &internodeInfo) {
                                                                                  return treeEntity ==
                                                                                         internodeInfo.m_plant;
                                                                              }
    );
    PlantManager::GetInstance().m_internodeQuery.ToComponentDataArray<InternodeInfo, InternodeInfo>(internodeInfos,
                                                                                                    [treeEntity](
                                                                                                            const InternodeInfo &internodeInfo) {
                                                                                                        return treeEntity ==
                                                                                                               internodeInfo.m_plant;
    ...
    for (auto &i : internodes) {
        ...
        node->append_attribute(
                doc.allocate_attribute("id", doc.allocate_string(std::to_string(i.m_index - rootNodeIndex).c_str())));
        ...
        position->append_attribute(
                doc.allocate_attribute("y", doc.allocate_string(std::to_string(globalTransform[3].y).c_str())));
        position->append_attribute(
                doc.allocate_attribute("z", doc.allocate_string(std::to_string(globalTransform[3].z).c_str())));
        node->append_node(position);

        auto *distRoot = doc.allocate_node(rapidxml::node_element, "DistToRoot");
        distRoot->append_attribute(doc.allocate_attribute("value", doc.allocate_string(
        ...
        thickness->append_attribute(doc.allocate_attribute("value", doc.allocate_string(
        ...
        maxLength->append_attribute(doc.allocate_attribute("value", doc.allocate_string(
                std::to_string(internodeStatistics.m_longestDistanceToAnyEndNode).c_str())));
        ...
        parent->append_attribute(
                doc.allocate_attribute("id", doc.allocate_string(std::to_string(parentIndex - rootNodeIndex).c_str())));
        parent->append_attribute(
                doc.allocate_attribute("thickness", doc.allocate_string(std::to_string(thicknessVal).c_str())));
        ...
        if (EntityManager::GetChildrenAmount(i) != 0) {
            ...
            EntityManager::ForEachChild(i, [children, &doc, rootNodeIndex](Entity child) {
                                            ...
                                            childNode->append_attribute(doc.allocate_attribute("id", doc.allocate_string(
                                                    std::to_string(child.m_index - rootNodeIndex).c_str())));
    ...
    for (auto &i : internodes) {
        ...
        for (auto &leafTransform : internodeData->m_leavesTransforms) {
            ...
            nodeAtt->append_attribute(
                    doc.allocate_attribute("id", doc.allocate_string(std::to_string(i.m_index).c_str())));
            leaf->append_node(nodeAtt);

            auto *posAtt = doc.allocate_node(rapidxml::node_element, "Center");
            posAtt->append_attribute(
                    doc.allocate_attribute("x", doc.allocate_string(std::to_string(nodePos.x).c_str())));
            posAtt->append_attribute(
                    doc.allocate_attribute("y", doc.allocate_string(std::to_string(nodePos.y).c_str())));
            posAtt->append_attribute(
                    doc.allocate_attribute("z", doc.allocate_string(std::to_string(nodePos.z).c_str())));
            ...
            frontAtt->append_attribute(doc.allocate_attribute("x", doc.allocate_string(
                    std::to_string((rotation * glm::vec3(0, 0, -1)).x).c_str())));
            frontAtt->append_attribute(doc.allocate_attribute("y", doc.allocate_string(
                    std::to_string((rotation * glm::vec3(0, 0, -1)).y).c_str())));
            frontAtt->append_attribute(doc.allocate_attribute("z", doc.allocate_string(
            ...
            leftAtt->append_attribute(doc.allocate_attribute("x", doc.allocate_string(
                    std::to_string((rotation * glm::vec3(1, 0, 0)).x).c_str())));
            leftAtt->append_attribute(doc.allocate_attribute("y", doc.allocate_string(
                    std::to_string((rotation * glm::vec3(1, 0, 0)).y).c_str())));
            leftAtt->append_attribute(doc.allocate_attribute("z", doc.allocate_string(
            ...
            centerAtt->append_attribute(
                    doc.allocate_attribute("x", doc.allocate_string(std::to_string(leafTransform[3].x).c_str())));
            centerAtt->append_attribute(
                    doc.allocate_attribute("y", doc.allocate_string(std::to_string(leafTransform[3].y).c_str())));
            centerAtt->append_attribute(
                    doc.allocate_attribute("z", doc.allocate_string(std::to_string(leafTransform[3].z).c_str())));
            ...
            distAtt->append_attribute(doc.allocate_attribute("value", doc.allocate_string(
                    std::to_string(glm::distance(nodePos, glm::vec3(leafTransform[3]))).c_str())));
