...
void DefaultFoliageGenerator::OnCreate() {
    ...
    if (!m_leafSurfaceTex) m_leafSurfaceTex = AssetManager::LoadTexture(AssetManager::GetAssetRootPath() +
...
void DefaultFoliageGenerator::OnGui() {
    ...
    ImGui::DragFloat2("Leaf Size XY", static_cast<float *>(static_cast<void *>(&m_defaultFoliageInfo.m_leafSize)),
                      0.01f);
