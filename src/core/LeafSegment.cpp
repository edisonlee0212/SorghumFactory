...
glm::vec3 LeafSegment::GetPoint(float angle)
{
	if (m_theta < 90.0f)
	{
		auto midRibMaxHeight = m_leafHalfWidth / 8.0f;
		auto midRibMaxAngle = m_theta / 5.0f;
		...
		auto distanceToCenter = m_radius * glm::cos(glm::radians(glm::abs(angle)));
		auto actualHeight = (m_radius - distanceToCenter) * (angle < 0 ? m_leftFlatness : m_rightFlatness);
		auto maxHeight = m_radius * (1.0f - glm::cos(glm::radians(glm::abs(m_theta))));
		auto center = m_position + (m_radius - m_leafHalfWidth) * m_up;
		auto direction = glm::rotate(m_up, glm::radians(angle), m_front);
		float compressFactor = glm::pow(actualHeight / maxHeight, angle < 0 ? m_leftFlatnessFactor : m_rightFlatnessFactor);
